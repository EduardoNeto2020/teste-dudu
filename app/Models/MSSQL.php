...
class MSSQL
{
     ...
     */
    private function retornaOrigemDados($tipofiltro): string
    {
        try {
        ...
        try {
            $this->isSP = collect(DB::select("select count(1) isSP
                                    ...
                                    where
                                        " . $str))[0]->isSP;
            if ($this->isSP == 1) {
                return collect(DB::select("exec fwork.getCamposProcedure '$schema.$tabela'"));
            } else {
